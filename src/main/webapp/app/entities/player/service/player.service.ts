...
type RestOf<T extends IPlayer | NewPlayer> = Omit<T, 'birthDate'> & {
...
export class PlayerService {
  ...
  protected convertDateFromClient<T extends IPlayer | NewPlayer | PartialUpdatePlayer>(player: T): RestOf<T> {
    return {
      ...player,
      birthDate: player.birthDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPlayer: RestPlayer): IPlayer {
    return {
      ...restPlayer,
      birthDate: restPlayer.birthDate ? dayjs(restPlayer.birthDate) : undefined,
    };
  }
